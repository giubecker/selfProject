...
export const Questionnaire = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:9002/formQuestions')
        .then((response) => {
      ...
      useEffect(() => {
        axios.get('http://localhost:9002/questionnaires')
        .then((response) => {
...
<div className="teste1">            <label key={formQuestion.title} htmlFor='title'>{formQuestion.id}- {formQuestion.title} </label><br/></div>
<div className="teste2">            <input key={index} id={formQuestion.id} name={formQuestion.title} type='text' onChange={onChange}/><br/></div>
        </div>

))};
        <div>
            <button className="button" type='submit'> Enviar </button>   <button className="button"> Cancelar </button>
        </div>
    </div>
</form>
